...
flags.DEFINE_string(
    "base_dir", "results", "The directory with the predicted masks.")
...
def mask_to_submission_strings(image_filename, mask_dir=None):
    ...
    if not match:
        raise ValueError(f"Filename {image_filename} does not contain any digits.")
...
def main(_):
    if not os.path.exists(FLAGS.base_dir):
        raise FileNotFoundError(f"The directory {FLAGS.base_dir} does not exist.")
